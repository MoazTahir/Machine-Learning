"""Training utilities for the PyTorch sparse autoencoder."""
...
def _run_epoch(
    ...
    for batch, _ in loader:
        ...
        kl_term = kl_divergence(config.sparsity_target, latent)
        loss = recon_loss + config.sparsity_weight * kl_term
...
def train_sparse_autoencoder(config: SparseConfig, model: nn.Module) -> Dict[str, List[float]]:
    ...
    best_val = float("inf")
    artifact_path = config.artifact_dir / "sparse_autoencoder.pt"
