"""Dataset utilities for the TensorFlow vanilla autoencoder."""
...
def load_datasets(config: AutoencoderConfig) -> Tuple[tf.data.Dataset, tf.data.Dataset]:
    path = config.data_dir / "fashion-mnist.npz"
    (x_train, _), (x_test, _) = tf.keras.datasets.fashion_mnist.load_data(path=str(path))

    train_ds = tf.data.Dataset.from_tensor_slices(_prepare(x_train))
    train_ds = train_ds.shuffle(10_000, seed=config.seed, reshuffle_each_iteration=True)
    train_ds = train_ds.batch(config.batch_size).prefetch(tf.data.AUTOTUNE)

    val_ds = tf.data.Dataset.from_tensor_slices(_prepare(x_test))
