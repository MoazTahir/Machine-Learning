"""Training pipeline for elastic net regression."""
...
class ElasticNetPipeline:
    """Encapsulates preprocessing, fitting, and artefact persistence."""
    ...
    def _sparsity_ratio(self) -> float:
        ...
        if coefs is None:
            return 0.0
        zero_count = float(np.sum(np.isclose(coefs, 0.0)))
        return zero_count / float(coefs.size)
...
def train_and_persist(
    ...
    pipeline = ElasticNetPipeline(config)
