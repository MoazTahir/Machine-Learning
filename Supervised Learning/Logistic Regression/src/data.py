...
def load_dataset(config: LogisticRegressionConfig = CONFIG) -> pd.DataFrame:
    """Load the heart disease dataset from disk."""
    return pd.read_csv(config.data_path)


def build_features(
    df: pd.DataFrame, config: LogisticRegressionConfig = CONFIG
...
def train_validation_split(
    config: LogisticRegressionConfig = CONFIG,
