"""Data loading utilities for the breast cancer random forest classifier."""
...
def _fetch_dataset(config: RandomForestClassificationConfig) -> pd.DataFrame:
...
def load_dataset(config: RandomForestClassificationConfig = CONFIG) -> pd.DataFrame:
    """Load the dataset from disk, fetching it from scikit-learn if missing."""
...
def build_features(
    df: pd.DataFrame, config: RandomForestClassificationConfig = CONFIG
...
def train_validation_split(
    config: RandomForestClassificationConfig = CONFIG,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
    """Return a stratified train/validation split."""
