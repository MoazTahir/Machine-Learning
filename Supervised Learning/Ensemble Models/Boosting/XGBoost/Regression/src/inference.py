"""Inference utilities for the California housing XGBoost regressor."""
...
from .pipeline import CaliforniaXGBoostPipeline, train_and_persist


class XGBoostRegressionRequest(BaseModel):
...
class XGBoostRegressionResponse(BaseModel):
    """Structured response returned by the XGBoost regressor."""
...
class XGBoostRegressionService:
    """High-level service for XGBoost regression inference."""
    ...
    def _ensure_model(self) -> xgb.XGBRegressor:
        ...
        model: xgb.XGBRegressor = joblib.load(self.config.model_path)
        self._model = model
        return model
    ...
    def _load_learning_curve(self) -> list[dict[str, float]] | None:
        if not self.config.learning_curve_path.exists():
            return None
        with self.config.learning_curve_path.open("r", encoding="utf-8") as fh:
            return json.load(fh)

    def predict(self, payload: XGBoostRegressionRequest) -> XGBoostRegressionResponse:
        model = self._ensure_model()
        row = np.asarray([[getattr(payload, feature) for feature in self.config.feature_columns]])
        prediction = float(model.predict(row)[0])
        response = XGBoostRegressionResponse(
...
def get_service(
    ...
    return XGBoostRegressionService(config)
