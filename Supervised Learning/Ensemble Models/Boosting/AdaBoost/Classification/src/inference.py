"""Inference utilities for the breast cancer AdaBoost classifier."""
...
from .pipeline import BreastCancerAdaBoostPipeline, train_and_persist


class AdaBoostClassificationRequest(BaseModel):
...
class AdaBoostClassificationResponse(BaseModel):
    """Structured response returned by the AdaBoost classifier."""
...
class AdaBoostClassificationService:
    """High-level service for AdaBoost breast cancer inference."""

    def __init__(self, config: AdaBoostClassificationConfig | None = None) -> None:
        self.config = config or CONFIG
        self._pipeline: BreastCancerAdaBoostPipeline | None = None

    def _load_or_train(self) -> BreastCancerAdaBoostPipeline:
        if self._pipeline is not None:
            return self._pipeline
        if not self.config.model_path.exists():
            train_and_persist(self.config)
        pipeline = BreastCancerAdaBoostPipeline(self.config)
    ...
    def predict(self, payload: AdaBoostClassificationRequest) -> AdaBoostClassificationResponse:
        pipeline = self._load_or_train()
        model = pipeline.pipeline
        if model is None:
            raise RuntimeError("Pipeline failed to load for inference.")
        feature_vector = np.asarray([[getattr(payload, feature) for feature in self.config.feature_columns]])
        prediction = model.predict(feature_vector)[0]
        probabilities = model.predict_proba(feature_vector)[0]
        class_list = list(model.classes_)
        positive_idx = class_list.index(self.config.positive_label)
        class_probabilities = {str(label): float(prob) for label, prob in zip(class_list, probabilities)}
        response = AdaBoostClassificationResponse(
            predicted_label=str(prediction),
            probability_of_malignant=float(probabilities[positive_idx]),
...
def get_service(
    ...
    return AdaBoostClassificationService(config)
