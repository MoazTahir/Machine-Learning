"""Inference utilities for the wine stochastic gradient boosting classifier."""
...
from .pipeline import WineStochasticGradientBoostingPipeline, train_and_persist


class StochasticGradientBoostingClassificationRequest(BaseModel):
...
class StochasticGradientBoostingClassificationResponse(BaseModel):
...
class StochasticGradientBoostingClassificationService:
    """High-level service that wraps stochastic gradient boosting inference."""

    def __init__(self, config: StochasticGradientBoostingClassificationConfig | None = None) -> None:
        self.config = config or CONFIG
        self._pipeline: WineStochasticGradientBoostingPipeline | None = None

    def _load_or_train(self) -> WineStochasticGradientBoostingPipeline:
        if self._pipeline is not None:
            return self._pipeline
        if not self.config.model_path.exists():
            train_and_persist(self.config)
        pipeline = WineStochasticGradientBoostingPipeline(self.config)
    ...
    def predict(self, payload: StochasticGradientBoostingClassificationRequest) -> StochasticGradientBoostingClassificationResponse:
        ...
        response = StochasticGradientBoostingClassificationResponse(
...
) -> StochasticGradientBoostingClassificationService:
    return StochasticGradientBoostingClassificationService(config)
