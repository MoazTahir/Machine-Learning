"""Inference utilities for the California housing stochastic gradient boosting regressor."""
...
from .pipeline import CaliforniaStochasticGradientBoostingPipeline, train_and_persist


class StochasticGradientBoostingRegressionRequest(BaseModel):
...
class StochasticGradientBoostingRegressionResponse(BaseModel):
    """Structured response returned by the stochastic gradient boosting regressor."""
...
class StochasticGradientBoostingRegressionService:
    """High-level service for stochastic gradient boosting regression inference."""

    def __init__(self, config: StochasticGradientBoostingRegressionConfig | None = None) -> None:
        self.config = config or CONFIG
        self._pipeline: CaliforniaStochasticGradientBoostingPipeline | None = None

    def _load_or_train(self) -> CaliforniaStochasticGradientBoostingPipeline:
        if self._pipeline is not None:
            return self._pipeline
        if not self.config.model_path.exists():
            train_and_persist(self.config)
        pipeline = CaliforniaStochasticGradientBoostingPipeline(self.config)
    ...
    def predict(self, payload: StochasticGradientBoostingRegressionRequest) -> StochasticGradientBoostingRegressionResponse:
        ...
        response = StochasticGradientBoostingRegressionResponse(
...
def get_service(
    config: StochasticGradientBoostingRegressionConfig | None = None,
) -> StochasticGradientBoostingRegressionService:
    return StochasticGradientBoostingRegressionService(config)
