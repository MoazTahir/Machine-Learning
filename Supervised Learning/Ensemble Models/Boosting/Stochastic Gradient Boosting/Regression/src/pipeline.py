"""Training utilities for the California housing stochastic gradient boosting regressor."""
...
class CaliforniaStochasticGradientBoostingPipeline:
    ...
    def _write_learning_curve(
        ...
        entries = []
        for iteration, prediction in enumerate(staged, start=1):
            rmse = float(np.sqrt(mean_squared_error(y_val, prediction)))
            entries.append({"iteration": iteration, "rmse": rmse})
        if not entries:
            return
...
def train_and_persist(
    ...
    pipeline = CaliforniaStochasticGradientBoostingPipeline(config)
