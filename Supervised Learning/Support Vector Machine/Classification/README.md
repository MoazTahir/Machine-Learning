...
2. **Train the model and persist artefacts**
   ...
   ```bash
   python "Supervised Learning/Support Vector Machine/Classification/src/train.py"
   ```
...
5. **Explore the companion notebook**
   ...
   Open `notebooks/svm_classification.ipynb` to step through dataset inspection, margin visualisation, hyperparameter experiments, and post-hoc diagnostics that mirror the scripted pipeline.
...
├── demo.py
├── notebooks/
│   └── svm_classification.ipynb # Exploratory notebook mirroring src/
└── src/
   ├── __init__.py
   ├── config.py
   ├── data.py
...
`notebooks/svm_classification.ipynb` walks through:
